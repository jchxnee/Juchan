...
public class MapRun {
	...
	public static void main(String[] args) {
		...
		//-> 객체를 추가하고자할 때 공통적으로 add메소드를 이용한다.
		
		//Map계열은 Collection을 구현한 클래스가 아니다.
		//-> 객체를 추가할 때 put메소드를 사용(key, value 쌍으로 담는다.)
		
		//1. put(k,v) : map에 k,v쌍으로 값을 추가하는 메소드
		...
		//4. remove(key) => 해당 키값을 찾아서 그 값을 삭제
		hm.remove("도리토스");
		System.out.println(hm);
		...
		for(Object obj : keySet) {
			System.out.println("키 : " + obj + " 값 : " + hm.get(obj));
		}
		
		//2. entrySet()을 이용하는 방법
		//	entrySet() : Map인터페이스에서 제공하는 메소드, 맵에 저장된 모든 키-값 쌍을 set형태로 변환한 것
