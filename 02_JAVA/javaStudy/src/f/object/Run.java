...
public class Run {
	/*
	 	...
	 	2. 객체란 : 현실세계에서 독립적(목적이 있고 가치가 있는 것)으로 존재하는 모든것을 의미
	 	...
	 	5. 추상화 과정
	 		...
	 		2) 공통적인 속성 및 기능(이름,나이,주소,전화번호,키,몸무게,머리색,발사이즈,달리기속도,먹는다,달린다,잔다,화장실간다...)
	 	...
	 	6. 추상화한 객체를 프로그램에 적용시키기
	 		1) "변수"만으로 프로그램을 만든다.
	 			...
	 			name1 : 최지원, age1 : 55
	 			name2 : 김지원, age2 : 50    -> name1과 age1이 하나라는 보장이 없다.
	 		2) "배열로 프로그램을 만든다.
	 	 	...
	 	 	-> 자바에서는 구조체개념에 좀더 확장해서 기능(메소드)을 추가한다 = 클래스
	 */
