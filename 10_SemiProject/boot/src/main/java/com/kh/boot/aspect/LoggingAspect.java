...
/*
    스프링 애플리케이션은 대부분의 경우에서 MVC패턴에 따라서 Web Layer -> Business Layer -> Data layer
...
/*
        <시점>
        @Before : 대상메소드 실행 전에 Advice(추가기능)가 실행된다

        @After : 대상메소드 실행 후에 Advice(추가기능)이 실행된다

        @AfterReturning : 대상메소드가 정상적으로 반환된 후에 Advice(추가기능)

        @AfterThrowing : 대상메소드가 비정상적으로 반환(예외를 반환)된 후에 Advice(추가기능)

        @Around : 대상메소드를 감싸서 메소드 호출 전후에 Advice(추가기능) 실행

        @PointCut
        - Advice를 적용할 JoinPoint들을 정의하는데 사용된다.
        - 해당 AOP를 실행할 특정 메소드나 클래스를 지정
     */
...
public class LoggingAspect {
    ...
    //com.kh.boot.RESTController패키지의 모든 클래스의 모든 메소드에서 전부 적용하겠다.
