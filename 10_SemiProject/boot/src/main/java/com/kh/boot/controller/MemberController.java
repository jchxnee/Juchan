...
public class MemberController {
    ...
    /*
        ...
        필드주입방식
        스프링 컨테이너가 객체를 생성 후, @Autiwired붙은 필드에 의존성을 주입해주는 방식
        장점 : 간결하다. 따로 생성자나 setter를 작성하지 않아도 된다.
        단점 : 테스트가 어려움(필드주입방식은 객체생성시 의존성이 주입되지 않고 bean에서 생성 후 주입받는 방식이기때문에
    ...
    /*
      ...
      1. HttpServletRequest를 활용해서 전달값을 가져옴
         ...
         스프링컨테이너가 해당 메소드를 호출할 때 자동으로 객체생성해서 매개변수로 주입
     */
    /*
    ...
    /*
     ...
     request.getParameter(키)로 value를 추출하는 역할을 대신 해주는 어노테이션
     요청 parameter의 key값과 동일하게 매개변수명을 설정해주면 @RequestParam어노테이션 생략할 수 있음
     */
    ...
    */
    ...
    public String login(@ModelAttribute Member member, HttpSession session) {
        ...
        //url재요청을 원할시 return내용을 redirect:재요청url로 해주면 된다.
        return "redirect:/";
    }
    ...
    public String insertMember(Member member, HttpSession session, Model model) {
        /*
          age값은 int로 필드를 구성할 경우 빈 문자열이 전달되면 형변환 과정에서 400에러가 발생
