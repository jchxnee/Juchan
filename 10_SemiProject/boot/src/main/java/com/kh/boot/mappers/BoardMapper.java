...
public interface BoardMapper {
    ...
    //댓글조회
    ArrayList<Reply> selectReplyList(@Param("bno") int bno);

    int updateBoard(Board b);

    ArrayList<Board> selectTopList(RowBounds rowBounds);

    int insertPhotoBoard(Board board);

    int selectPhotoCount();

    ArrayList<Board> selectPhotoList(RowBounds rowBounds);

    Board selectPhoto(@Param("bno") int bno);
}
