/*
   ...
   <SELECT>
   SELECT 가지고고오고싶은 정보 FROM 테이블;
   EX) SELECT 컬럼1, 컬럼2, 컬럼3... FROM 테이블;
*/
...
/*
    <컬럼에 별칭 지정하기>
    산술연산을 하게되면 컬럼명이 지저분해진다. 이때 컬럼명에 별칭을 부여해서
    깔끔하게 가져올 수 있다.
    [표현법]
...
SELECT EMP_ID || EMP_NAME || SALARY AS 황인태
FROM EMPLOYEE;
...
*/
-- EMPLOYEE에서 잡코드 중복을 제거한 값 출력
SELECT DISTINCT JOB_CODE FROM EMPLOYEE ;


--SELECT DISTINCT JOB_CODE, DISTINCT DEPT_CODE
--위처럼 작성시 에러가 발생한다. DISTINCT는 한번만 사용 가능
...
/*
  <WHERE 절>
  조회하고자하는 테이블로부터 특정 조건에 만족하는 데이터만 조회할 때 사용한다.
  조건식에서도 다양한 연산자를 사용할 수 있다.
  
  [표현법]
  SELECT 컬럼, 컬럼, 컬럼
  FROM 테이블명
  WHERE 조건식 EX) JOB_CODE = 10;
...
SELECT * FROM SAL_GRADE WHERE MIN_SAL >1000000 AND (MAX_SAL<4000000);
--------혼자 실습---------------------------
...
SELECT PHONE, HIRE_DATE, EMAIL FROM EMPLOYEE WHERE SALARY<= 4000000 OR(SALARY >= 6000000);
...
/*
   ...
   조건A OR 조건B -> 조건A나 조건 B가 둘 중 하나만 만족해도 값을 참으로 한다.
   
   <BETWEEN AND>
   조건식에 사용되는 구문
   몇이상 몇이하인 범위에 대한 조건을 제시할 때 주로 사용하는 연산자(이상,이하만 가능) -- 미만,초과 불가능
   [표현법]
   비교대상 컬럼 BETWEEN 하한값 AND 상한값;
*/

-- 급여가 350만원 이상 600만원 이하의 모든 사원의 사원명,사번,급여 조회

SELECT EMP_NAME, EMP_ID, SALARY 
FROM EMPLOYEE 
--WHERE SALARY >= 3500000 AND(SALARY <=6000000);
--WHERE SALARY BETWEEN 3500000 AND 6000000;
WHERE SALARY NOT BETWEEN 3500000 AND 6000000; -- BETWEEN조건문에 대한값이 아닌값을 반환
--NOT : 논리부정 연산자
...
SELECT * 
FROM EMPLOYEE
--WHERE BONUS = NULL; -- NULL을 사용할 수 없다.
--WHERE BONUS IS NULL; -- 보너스가 NULL값인 모든값 반환
WHERE BONUS IS NOT NULL; -- 보너스가 NULL값이 아닌 모든값 반환
...
/*
   ...
   EX) 비교할 대상 컬럼 LIKE '문자%' : 비교할대상컬럼값 중에서 해당문자로 시작하는 값들을 전부 반환
       비교할 대상 컬럼 LIKE '%문자' : 비교할대상컬럼값 중에서 해당문자로 끝나는 값들을 전부 반환
       비교할 대상 컬럼 LIKE '%문자%' : 비교할대상컬럼값 중에서 해당문자가 포함된 값들을 전부 반환
   
   2. '_' : 1글자를 대체검색할 때 사용
   EX) 비교할 대상 컬럼 LIKE '_문자' : 비교할대상컬럼값 문자앞에 아무글자나 딱 한글자가 있는 값을 반환
       비교할 대상 컬럼 LIKE '문자_' : 비교할대상컬럼값 문자뒤에 아무글자나 딱 한글자가 있는 값을 반환
       비교할 대상 컬럼 LIKE '_문자_' : 비교할대상컬럼값 문자앞뒤에 아무글자나 딱 한글자가 있는 값을 반환
...
FROM EMPLOYEE
WHERE EMP_NAME LIKE '_하_'; -- _의 갯수만큼 한글자
...
/*
   <IN>  ANY , ALL
   WHERE절에 비교대상 컬럼값이 내가 제시한 목록중에 일치하는 값이 있는지 검사하는 문법
   
   [표현법]
   비교대상 컬럼 IN ('값', '값', '값' ..)
*/

--부서코드가 D6이거나 D8이거나 D5인 부서원들의 이름, 부서코드, 급여조회
...
/*
    ...
    5. BETWEEN A AND B
    6. NOT
    7. AND   --EX) A AND B OR A AND C 이면 AND 2개 먼저 실행 후 나중 OR 실행
    8. OR
*/
...
FROM EMPLOYEE
WHERE EMP_NAME LIKE '%연';
--WHERE EMP_NAME LIKE '__연';    --이름이 3글자가 아니면 출력 불가능
...
--5. 사수가 없고(MANAGER)가 없고 부서배치도 받지 않은 사원들의 사원명, 사번, 부서코드 조회
