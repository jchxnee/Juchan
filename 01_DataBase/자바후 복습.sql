...
SELECT TO_CHAR(DATE '2020-12-25','DAY') AS "크리스마스날"
FROM DUAL;
...
--4. 현재 법학과 교수 중 가장 나이가 많은 사람부터 이름을 확인할 수 있는 SQL


--5. 2004년 2학기에 'C3118100' 과목을 수강한 학생들의 학점을 조회 학점이 높은 학생부터 학점이 같으면 학번이 낮은학생


--6.학생 번호, 학생 이름, 학과 이름 학생 이름으로 오름차순 정렬 출력

--7. 춘 기술대학교의 과목 이름과 과목의 학과 이름을 출력하는 작성
...
--11. 학번이 A313047인 학생이 학교에 나오고 있지 않다. 지도 교수에게 내용을 전달하기위한
    -- 학과이름, 학생 이름과 지도 교수 이름이 필요하다


--12. 2007년도에 인간관계론 과목을 수강한 학생을 찾아 학생이름과 수강학기를 표시하는 SQL


--13. 예체능 계열 과목 중 과목 담당교수를 한 명도 배정받지 못한 과목을 찾아 그과목 이름과 학과 이름을 출력하는 SQL
;
...
--15. 휴학생이 아닌 학생 중 평점이 4.0이상인 학생을 찾아 그 학생의 학번, 이름, 학과 이름, 평점을 출력하는 SQL                  
...
--19. 춘 기술대학교의 "환경조경학과"가 속한 같은 계열 학과들의 학과 별 전공과목 평점을 파악하기 위한 적절한 SQL문
...
--2. 과목 구분을 저장할 테이블을 만들려고한다. 다음과 같은 테이블을 작성하시오

    

--3. TB_CATAGORY 테이블의 NAME 컬럼에 PRIMARY KEY를 생성하시오
-- (KEY 이름을 생성하지 않아도 무방함. 만일 KEY 이름을 지정하고자 한다면 이름은 본인이 알아서 적당한 이름을 사용)

--4. TB_CLASS_TYPE 테이블의 NAME 컬럼에 NULL값이 들어가지 않도록 속성을 변경

--5. 두 테이블에서 컬럼 명이 NO인 것은 기존 타입을 유지하면서 크기는 10으로,
--컬럼명이 NAME인 것은 마찬가지로 기존 타입을 유지하면서 크기 20으로 변경하시오


--6. 두 테이블의 NO컬럼과 NAME컬럼의 이름을 각 각 TB_를 제외한 테이블 이름이 앞에 붙은 형태로 변경한다.


--7. TB_CATAGORY테이블과 TB_CLASS_TYPE테이블의 PRIMARY KEY 이름을 다음과 같이 변경하시오
...
--9. TB_DEPARTMENT의 CATEGORY 컬럼이 TB_CATEGORY테이블의 CATEGORY_NAME 컬럼을 부모값으로 참조하도록
-- FOREIGN KEY를 지정하시오. 이때 KEY이름은 FK_테이블이름_컬럼이름으로 지정한다. (EX. FK_DEPARTMENT_CATEGORY)


--10. 춘 기술대학교 학생들의 정보만이 포함되어 있는 학생일반정보 VIEW를 만들고자 한다.
--아래 내용을 참고하여 적절한 SOL 문을 작성하시오.
...
--13. 위에서 생성한 학생일반정보 View를 통해서 학번이 A213046 인 학생의 이름을 본인 이름으로 변경하는 SOL 문을 작성하시오.
...
--아래 내용을 참고하여 적절한 SOL 문을 작성하시오. (서브쿼리를 이용하시오)


--3. 국어국문학과 학생들의 정보만이 포함되어 있는 학과정보 테이블을 만들고자 한다.
--아래 내용을 참고하여 적절한 SOL 문을 작성하시오. (힌트 : 방법은 다양함, 소신껏 작성하시오)


--4. 현 학과들의 정원을 10% 증가시키게 되었다. 이에 사용할 SOL 문을 작성하시오. 
...
--이 내용을 반영할 적절한 SOL 문장을 작성하시오.
--(예. 830530-2124663 =〉 830530 )


--7. 의학과 김명훈 학생은 2005년 1학기에 자신이 수강한 피부생리학' 
--점수가 잘못되었다는 것을 발견하고는 정정을 요청하였다. 
--담당 교수의 확인 받은 결과 해당 과목의 학점을 3.5로 변경키로 결정되었다. 적절한 SOL 문을 작성하시오.
