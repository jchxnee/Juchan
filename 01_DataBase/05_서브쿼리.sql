...
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
WHERE JOB_CODE IN (SELECT JOB_CODE    --IN을 사용하여 하나라도 일치하는 값 반환
                   FROM EMPLOYEE 
...
--다중열 서브쿼리
--튜플 비교 : 두개 이상의 열(컬럼)을 조합해서 비교하는 방식
SELECT EMP_NAME, DEPT_CODE, JOB_CODE, HIRE_DATE
FROM EMPLOYEE
...
--> 인라인뷰를 주로 사용하는 경우 -> TOP-N 분석 : 상위몇개만 조회
...
--별칭을 부여해야 한다.
SELECT SALARY_AVG
FROM (SELECT DEPT_CODE,  ROUND(AVG(SALARY)) SALARY_AVG
      FROM EMPLOYEE
      GROUP BY DEPT_CODE
...
SELECT *
FROM (SELECT DEPT_CODE, SUM(SALARY), ROUND(AVG(SALARY)) AS AVG_SALARY, COUNT(*)
      FROM EMPLOYEE
      GROUP BY DEPT_CODE)
WHERE AVG_SALARY > 2700000;
