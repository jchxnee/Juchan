...
--각 부서별로 급여 총합
--그룹 : 부서
SELECT DEPT_CODE, SUM(SALARY) --GROUP BY 절을 사용하지않으면 DEPT_CODE 표현 불가능
FROM EMPLOYEE
GROUP BY DEPT_CODE;
...
--각 직급별 총 사원수, 보너스를 받은 사람 수, 급여합, 평균급여, 최저시급, 최고급여 (정렬 = 직급 내림차순)
...
--각 부서별 평균 급여가 300만원 이상인 부서들만(부서코드, 평균급여)
SELECT DEPT_CODE, ROUND(AVG(SALARY))
FROM EMPLOYEE
GROUP BY DEPT_CODE
...
/*
   ...
   HAVING 그룹에 대한 조건식                     --4
   ORDER BY 정렬기준컬럼                        --6
   (오라클에는 존재X) LIMIT 숫자  -숫자만큼 잘라서 보여줌 --정렬보다 더 뒷순번
*/
