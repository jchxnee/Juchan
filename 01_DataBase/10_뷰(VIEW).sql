...
-- GRANT CREATE VIEW TO C##KH; --뷰 생성권한(관리자계정)

SELECT * FROM VW_EMPLOYEE;

--실제 실행되는 것은 서브쿼리로(인라인뷰)로 실행된다고 볼 수 있다.
SELECT * 
...
--CREATE OR REPLACE
--VIEW가 없을때는 생성, 이미 존재한다면 수정할 수 있다.
...
SELECT * FROM VW_JOB; 
SELECT * FROM JOB;  --VW_JOB 변경값과 동일하게 값이 변경
...
/*
    ...
    [상세표현식]
    CREATE [OR REPLACE] [FORCE | NOFORCE] VIW 뷰명
    AS 서브쿼리
    [WITH CHECK OPTION]
    [WITH READ ONLY]
...
--서브쿼리에 기술된 테이블이 존재하지 않아도 뷰가 우선은 생성이 된다(실행 X)
...
UPDATE VW_EMP
SET SALARY = 2000000
WHERE EMP_ID = 200;              --EMP_ID가 200인 사원이 뷰에서 제외된다
...
UPDATE VW_EMP
SET SALARY = 2000000
WHERE EMP_ID = 200;     --뷰의 WITH CHECK OPTION의 조건에 위배 됩니다
...
DELETE 
FROM VW_EMP
WHERE EMP_ID = 200;  --읽기 전용 뷰에서는 DML 작업을 수행할 수 없습니다