/*
   <INDEX>
   *인덱스 : 데이터를 빠르게 검색하기 위한 데이터의 정렬과, 탐색같은 DBMS의 성능향상을 목적으로
            생성하는 객체
    - 검색속도가 빨라진다. (SELECT문에서 WHERE조건으로 데이터를 빠르게 검색)
    단, 모든 경우에 향상되는 것은 아니다.(INSERT, UPDATE, DELETE 작업시, 
        인덱스도 함께 업데이트되어 성능저하될 수 있음)
...
------------------------------------------------------------------------------------
 COST : 실행 예상 비용(CPU 자원을 얼마나 소비하는가) -> 값이 낮을수록 적은 비용으로 검색을 수행할 수 있음.
...
------------------------------------------------------------------------------------------------
INDEX (UNIQUE) SCAN : 인덱스 객체를 참조(UNIQUE)하여 탐색 후 결과를 도출하게 될 것을 예상한다.
TABLE ACCESS (BY INDEX ROWID) : 인덱스 객체로 참조한 INDEX ROWID로 탐색하여 결과를 도출하게 될 갓을 예상한다.
*/
...
| Id  | Operation         | Name           | Rows  | Bytes | Cost (%CPU)| Time     |
------------------------------------------------------------------------------------
|   0 | SELECT STATEMENT  |                |     8 |  1064 |   137   (1)| 00:00:01 |
|*  1 |  TABLE ACCESS FULL| USER_MOCK_DATA |     8 |  1064 |   137   (1)| 00:00:01 |
------------------------------------------------------------------------------------
*/
