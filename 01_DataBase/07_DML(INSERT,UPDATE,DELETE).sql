...
/*
    ...
    단, 기본값이 지정되어 있으면 NULL이아닌 기본값이 들어간다.
 */
...
--EMP_SALARY테이블에서 노옹철 사원의 급여를 100만으로변경
UPDATE EMP_SALARY
SET SALARY = 1000000
WHERE EMP_NAME = '노옹철';

--EMP_SALARY테이블에서 선동일 사원의 급여를 700만원, 보너스를 0.2로변경
UPDATE EMP_SALARY
   SET SALARY = 7000000,
         BONUS = 0.2
WHERE EMP_NAME = '선동일';

--EMP_SALARY테이블에서 전체사원의 급여를 기존 급여에 10프로 인상된 급액으로 변경해라.
UPDATE EMP_SALARY
...
--방명수 사원의 급여와 보너스값을 유재식 사원의 급여와 보너스값으로변경
UPDATE EMP_SALARY
