...
/*
   ...
   - ASC : 오름차순(작은 값으로 시작해서 점점 커지는 것) -> 기본값 DEFAULT
   - DESC : 내림차순(큰 값으로 시작해서 점점 작아지는 것)
...
FROM EMPLOYEE
--ORDER BY SALARY *12 DESC; --컬럼명   --정렬 ORDER BY는 항상 제일 마지막에 실행
--ORDER BY 연봉 DESC;       --별칭
...
*/
   ...
   SELECT INSTR('AABAACAABBAA', 'B', 1, 2) FROM DUAL; -- B를 1번째부터 찾지만 두번째 B위치 출력
   ...
   */
   ...
   SELECT SUBSTR('SHOWMETHEMONEY',7) FROM DUAL; --7번째 위치부터 끝까지 반환
   SELECT SUBSTR('SHOWMETHEMONEY',5,2 ) FROM DUAL; --5번째 위치부터 2글자까지 반환
   SELECT SUBSTR('SHOWMETHEMONEY',1,6) FROM DUAL; --1번째 위치부터 6글자까지 반환
   SELECT SUBSTR('SHOWMETHEMONEY',-8, 3) FROM DUAL; --거꾸로 8번째 위치부터 3글자 반환
   
   SELECT EMP_NAME, EMP_NO, SUBSTR(EMP_NO, 8, 1) --성별 구분
   FROM EMPLOYEE;
   ...
   /*
      *LPAD / RPAD
      ...
      [표현법]
      LAPD/RPAD(STRING, 최종적으로 반환할 문자열의 길이, [덧붙이고자하는 문자])
   */

   --30만큼의 길이 중 EMAIL컬럼값은 오른쪽으로 정렬하고 나머지 부분은 공백으로 채운다.
   SELECT EMP_NAME, LPAD(EMAIL, 30,'#') -- 왼쪽 공백을 #으로 채움
   FROM EMPLOYEE;
   
   SELECT EMP_NAME, RPAD(EMAIL, 30,'@') -- 우측 공백을 @으로 채움
   FROM EMPLOYEE;
...
SELECT LTRIM('123123KH123', '123') FROM DUAL; --왼쪽부터라 KH를 만나면 그 이후에 123은 제거불가
SELECT LTRIM('ACABACCAKH', 'ABC') FROM DUAL; --ABC라는 문자가 아닌 각각의 A,B,C가 들어간 문자열 제거
SELECT RTRIM('SDSKHACABACCA', 'ABC') FROM DUAL;  --우측부터 ABC 각각의 문자를 제거하여 SDSKH 출력
/* 
   *TRIM
...
SELECT TRIM(BOTH 'Z' FROM 'ZZZZZZZZZKHZZZZZ') FROM DUAL; --양쪽
SELECT TRIM(LEADING 'Z' FROM 'ZZZZZZZZZKHZZZZZ') FROM DUAL; -- LTRIM 유사
SELECT TRIM(TRAILING 'Z' FROM 'ZZZZZZZZZKHZZZZZ') FROM DUAL; --RTRIM 유사
...
SELECT EMAIL, REPLACE(EMAIL, 'C##SERVER', 'KH') AS 변경이메일 --C##SERVER -> KH로 교체
FROM EMPLOYEE; 
...
SELECT ABS(-10), ABS(-5.4) FROM DUAL; -- 10, 5.4 출력
...
SELECT MOD(10,3) FROM DUAL;  --10을 3으로 나눈 나머지 1출력
SELECT MOD(10.9,3) FROM DUAL; -- 10.9를 3으로 나눈 나머지 1.9 출력
...
SELECT ROUND(123.456, 2) FROM DUAL; --둘째자리까지 표현하므로 셋째자리에서 반올림
SELECT ROUND(123.45678, 3) FROM DUAL; --셋째자리까지 표현 넷째자리에서 반올림
SELECT ROUND(123.656, -1) FROM DUAL; --음수로 감소할수록 소수점 앞자리로 이동, -1이므로 소수점기준 앞인 123에서 반올림하여 120출력

/*
   *CEIL
   올림처리를 위한 함수 -> 무조건 소수점 첫번째자리에서 반올림
   CEIL(NUMBER)
*/
...
/*
   ...
   TRUNC(NUMBER, [위치])
   FLOOR(NUMBER)  --위치 지정불가
*/

SELECT TRUNC(123.952) FROM DUAL;
SELECT TRUNC(123.952, 1) FROM DUAL; --소수점 첫번째자리 이후로 버림 123.9출력
SELECT TRUNC(123.952, -1) FROM DUAL; --음수이므로 소수점 앞에 한자리까지 버림 123에서 3 버림으로 120출력
...
-- *LASY_DAY(DATE) : 해당월의 마지막 날짜 구해서 반환
SELECT LAST_DAY(SYSDATE) FROM DUAL;

/* 
   *EXTRACT : 특정 날짜로부터 년|월|일 값을 추출해서 반환하는 함수
   [표현법]
   EXTRACT(YEAR FROM DATE) : '년도'만 추출
...
SELECT TO_CHAR(12, '99999') FROM DUAL; -- 9의 자리수만큼 공간 확보(공백)
...
--날짜타입 -> 문자타입
SELECT SYSDATE FROM DUAL;
SELECT TO_CHAR(SYSDATE) FROM DUAL; --DATE값이 문자로 변환
SELECT TO_CHAR(SYSDATE, 'HH:MI:SS') FROM DUAL;
SELECT TO_CHAR(SYSDATE, 'PM HH:MI:SS') FROM DUAL; --AM, PM 어떤것을 사용하건 형식을 정해준다
SELECT TO_CHAR(SYSDATE, 'YYYY-MM-DD DAY DY') FROM DUAL;
SELECT TO_CHAR(SYSDATE, 'YYYY-MM-DD DAY DY HH:MI:SS') FROM DUAL;
SELECT TO_CHAR(SYSDATE, 'YYYY"년" MM"월" DD"일" DAY DY HH"시"MI"분"SS"초" ') FROM DUAL;
...
SELECT TO_CHAR(SYSDATE, 'YYYY'),
       ...
       TO_CHAR(SYSDATE, 'YYY'), --2024중 024 출력
       TO_CHAR(SYSDATE, 'YEAR') --영어로 출력
       FROM DUAL;

SELECT
 TO_CHAR(TO_DATE('24', 'RRRR'), 'YYYY') , --현재 세기(2000)으로 해석
 TO_CHAR(TO_DATE('60', 'RRRR'), 'YYYY'), -- 50년이상이므로 과거(한세기 전)로 해석
...
SELECT TO_DATE(611218) FROM DUAL; -- 기본적으로 연도의 2자리만 입력시 RR룰이 적용된다

SELECT TO_DATE(051010) FROM DUAL; -- 0으로 시작하는 숫자는 없다.
SELECT TO_DATE('051010') FROM DUAL; -- 문자타입으로는 가능

SELECT TO_DATE('020505 120500') FROM DUAL; -- 포맷을 정해주게 되어있음
SELECT TO_DATE('020505 230500', 'YYMMDD HH24MISS') FROM DUAL; -- 문자열로 바꾸는것이 아니라 화면에 시간은 출력 X
...
SELECT TO_NUMBER('00123123123123') FROM DUAL;

SELECT '1000' + '3000' FROM DUAL; --문자열 -> 숫자로 자동으로 형변환을 시도하여 계산이 됨
SELECT '100,000' + '30,000' FROM DUAL; --  ,로 인해 계산 불가
SELECT TO_NUMBER('100,000', '999,999') + TO_NUMBER('30,000', '999,999') FROM DUAL;

SELECT SALARY, NATIONAL_CODE
...
SELECT EMP_NAME, NVL(BONUS,0) -- 보너스가 NULL값이면 0으로 대체
FROM EMPLOYEE;

--전 사원의 이름, 보너스포함 연봉 조회
SELECT EMP_NAME, (SALARY + (SALARY * NVL(BONUS, 0))) * 12 --보너스가 NULL값이어도 연봉 출력
FROM EMPLOYEE;
...
SELECT NULLIF('123', '123') FROM DUAL; --  일치하므로 NULL 반환
SELECT NULLIF('123', '456') FROM DUAL; -- 일치하지 않으므로 123 반환
=======================================================
...
SELECT EMP_NAME, SALARY,  -- 자바의 IF문과 유사함
       CASE
 ...
 SELECT SUM(SALARY)
 FROM EMPLOYEE
 WHERE SUBSTR(EMP_NO, 8, 1) IN ('1','3');  --OR 절을 사용가능
 ...
 SELECT CEIL(AVG(SALARY)) -- 평균을 구해서 올림처리
 FROM EMPLOYEE;
 ...
 FROM EMPLOYEE; -- NULL값도 반환, COUNT는 NULL을 세지 않음
