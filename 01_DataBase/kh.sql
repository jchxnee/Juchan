...
--부서코드가 'D2'인 사원 또는 'D6'인 사원의 이름, 부서코드 조회(UNION) -> OR
...
--집합연산자는 컬럼타입(문자, 숫자, 날짜)만 일치하면 연산수행 가능하다.
...
-- GROUP BY ROLLUP(A) : 전체통계, 컬럼A에 따른 집계
...
-- GROUP BY ROLLUP(A, B, C) : 전체합, 컬럼 A에따른 집계, 컬럼(A,B)에 대한 집계, 컬럼(A,B,C)에 대한 집계
-- GROUP BY ROLLUP(A,(B, C) : 전체합, 컬럼 A에따른 집계, 컬럼 (A,(B,C))에 대한 집계
-- GROUP BY A, ROLLUP(B) : A그룹별, 전체합, B에따른 집계

2. CUBE
--조합가능한 모든 값에 대한 다차원 집계
-- GROUP BY CUBE(A) : 전체합계, 컬럼A에 대한 집계
-- GROUP BY CUBE(A, B) : 전체합계, 컬럼A에 대한 집계, 컬럼B에 대한 집계, 컬럼(A, B)에 대한 집계
SELECT DEPT_CODE, JOB_CODE, SUM(SALARY)
FROM EMPLOYEE
...
--SQL에서 특정 값을 기준으로 순위를 매기는 함수

--RANK() : 동순위 -> 건너뜀
--DENSE_RANK() : 동순위 -> 연속적으로 순위를 부여
...
-- PERCENT_RANK() -- 가장먼저 조회된 값부터 늦게 조회된 값까지 결과값이 아닌 행의 순서별 백분율
-- 순위-1/전체행개수-1
SELECT EMP_ID, EMP_NAME, SALARY,
...
--NTITLE() - 데이터를 N개의 그룹으로 균등하게 분할하는 함수
SELECT EMP_ID, EMP_NAME, SALARY,
...
/*
    계층형 쿼리
-- 내부조인관계(동일 테이블에 계층적으로 상위와 하위데이터가 포함된 데이터를 말함 -> 사수, 부사수)
...
--CONNECT_BY_ROOT 컬럼명을 사용하면 가장 최상위 부모값을 가져올 수 있음
